...
class BookScanner {
  ...
  async scanAppDirectory(showAlert = true): Promise<BookFile[]> {
    try {
      ...
      const epubFiles: File[] = await this.getEpubFilesInDirectory(BookScanner.BOOKS_DIR);
  ...
  async createBookFromFile(file: File): Promise<BookFile> {
    const { coverImage, author, title } = await this.extractBookMetadata(file.uri, file.name);
    return {
      name: file.name,
      uri: file.uri,
   ...
   */
  filterDuplicates(existingBooks: BookFile[], newBooks: BookFile[]): BookFile[] {
  ...
  try {
    ...
    const newBooks = await Promise.all(
      result.assets.map(async (asset) => {
        ...
        const destinationFile: File = new File(BookScanner.BOOKS_DIR,asset.name);
